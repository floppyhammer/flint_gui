...
void SpriteGuiRenderSystem::update(const std::weak_ptr<Coordinator> &p_coordinator, double dt) {
    ...
    for (auto const &entity : entities) {
        ...
        mvp.model = glm::scale(
            mvp.model,
            glm::vec3(transform.size.x / viewport_extent.x * 2.0f, transform.size.y / viewport_extent.y * 2.0f, 1.0f));
...
void Sprite2dRenderSystem::update(const std::weak_ptr<Coordinator> &p_coordinator, double dt) {
    ...
    for (auto const &entity : entities) {
        ...
        mvp.model = glm::scale(
            mvp.model,
            glm::vec3(sprite_width / viewport_extent.x * 2.0f, sprite_height / viewport_extent.y * 2.0f, 1.0f));
